...
Wired the three non-Convex sponsors to do real work and proved each live. AgentMail: per-user inbox provisioning (each signup mints its own inbox, idempotent via client_id) + real inbound (external Gmail → webhook → Warden held an SSN request) + real outbound (approve → in-thread reply sent). Firecrawl: v2 scrape enriches each first-seen counterpart domain (verified against greenhouse.io, acme.com). OpenAI: typed extraction on real inbound. All no-op safely without a key so the demo never errors.
